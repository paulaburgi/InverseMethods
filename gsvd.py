...
def csd(Q1,Q2):
    ...
    if k < min([n,p]):
        ...
        #print ST.shape
        #ST = add_zeros(ST,np.zeros([p,p-1])) # problem here
        ST = np.diag(ST)
        if k > 0: 
            S[0:k,j] = 0

        #print S.shape, ST.shape,n,r,i,j

        S[k:p,k:p]=ST # not sure about this ... maybe q
        #S[k+1:n,k+1:r] = ST # and here
        ...
        Q,R = scipy.linalg.qr(C[k:q,k:r]) # need Q here
        #print diagf(R)
        C[k:q,k:r] = diagf(R) # and here; also changed
        #print U.shape, U[:,i].shape, Q.shape, C.shape, n, r
        U[:,t] = np.dot(U[:,i],Q)
...
def diagp(Y,X,k):
    ...
    if(j != []):
        ...
        #print D.shape,Y.shape,Y[:,j].shape
        Y[:,j] = np.dot(Y[:,j],D.T) # DLH
        X[j,:] = np.dot(D,X[j,:])
    return Y, X
